...
# a data container for all pertinent information related to fares.
class FareEntry:
...
class Dispatcher:
    ...
    def _costFare(self, fare):
        ...
        if timeToDestination < 0:
            return 150
        return 3 * (25 + timeToDestination) / 0.9

    def calculateCost(self, origin, destination, **args):
        h = [(0, origin)]
        distances = {origin: 0}
        parent = {}
        s = set()
    ...
    # action. You should be able to do better than that using some form of CSP solver.
                ...
                for taxiIdx in self._fareBoard[origin][destination][time].bidders:
                    if len(self._taxis) > taxiIdx:
                        ...
                        if bidderNode is not None:
                            # ultimately the naive algorithm chosen is which taxi is the closest. This is patently
                            # unfair for several reasons, but does produce *a* winner.
                            # if winnerNode is None or self._parent.distance2Node(bidderNode, fareNode) \
                            #         < self._parent.distance2Node(winnerNode, fareNode):
                            if allocatedTaxi == -1 or (len(self.calculateCost(bidderLoc, origin))
                                                           + len(self.calculateCost(origin, destination))) \
                                    < shortestPath:
                                allocatedTaxi = taxiIdx
                                shortestPath = (len(self.calculateCost(bidderLoc, origin))
                                                           + len(self.calculateCost(origin, destination)))
                            # else:
                            # and after all that, we still have to check that somebody won, because any of the other
                            # reasons to invalidate the auction may have occurred.
                print(shortestPath, self._fareBoard[origin][destination][time].price)
                if allocatedTaxi >= 0 and shortestPath < self._fareBoard[origin][destination][time].price:
