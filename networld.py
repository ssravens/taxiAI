...
class Node:
    ...
    def dropoffFare(self, fare, direction, taxi):
        if direction in self._occupied.keys():
            # fares will only alight at their actual destination (naturally)
            # print(fare.destination, taxi_location , self._occupied[direction][1], self._parent.simTime)
            if fare is not None and fare.destination == taxi._loc.index and self._occupied[direction][1] >= self._parent.simTime:
                fare.dropOff()
                self._parent.completeFare(fare, taxi)
                return True
        return False
...
class NetWorld:
   ...
   '''
    ...
    def completeFare(self, fare, taxi):
        self._dispatcher.recvPayment(self, fare.price * 0.1)
        fare.taxi.recvMsg(taxi.FARE_PAY, **{'amount': fare.price * 0.9})
